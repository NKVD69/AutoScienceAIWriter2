#!/usr/bin/env python3
"""Verifie les garanties du bac a sable sous Windows (US-004, ADR-005).
...
Le point cle : les garanties DECLAREES different par niveau. Au niveau 1 (Wasm),
reseau et disque hors montage sont isoles. Au niveau 2 (natif Windows), ils ne
le sont PAS — les Job Objects bornent les ressources, pas le reseau. Que le code
natif JOIGNE le serveur local est donc ATTENDU, jamais un echec du script.

La sonde reseau vise un echo TCP local, sur 127.0.0.1, demarre par ce script :
rien ne quitte la machine, et la difference entre « pas de pile reseau » (niveau
1) et « pile presente » (niveau 2) est prouvee, non supposee.
...
def start_echo_server() -> tuple[int, socketserver.TCPServer]:
    """Echo TCP local. La sonde reseau s'y connecte : preuve locale, sans egress."""
...
def disk_probe(host_path: Path) -> str:
    chemin = str(host_path).replace("\\", "/")
    return (
        f"try:\n"
        f"    open({chemin!r}).read()\n"
...
async def run_level_2(secret: Path, port: int, tmp: Path) -> None:
    sb = select(SandboxOrigin.USER, SandboxMode.NATIVE, platform="win32")
    ...
    record(
        ...
        "reseau (declare)",
        "NON isole",
        f"guaranteed={r.network_isolation_guaranteed}",
        r.network_isolation_guaranteed is False,
    )

    r = await sb.run(network_probe(port), [], ResourceLimits(wall_seconds=15), tmp / "l2b")
    record(
        "2 natif", "reseau (sonde)", "pile presente", r.stdout.strip(), "NET_CONNECTED" in r.stdout
    )
...
async def main() -> int:
    if sys.platform != "win32":
        print("Ce script vise Windows. Sous Linux : check_sandbox_linux.py.")
        return 2
    ...
    print(f"\n{'Niveau':8} {'Sonde':{largeur}} {'Attendu':22} {'Obtenu':26} Etat")
    print("-" * (8 + largeur + 22 + 26 + 8))
    for level, probe, expected, obtained, ok in results:
        etat = "OK" if ok else "ECHEC"
        print(f"{level:8} {probe:{largeur}} {expected:22} {obtained:26} {etat}")

    conforme = all(ok for *_, ok in results)
    print(
        "\ncheck_sandbox_windows : "
