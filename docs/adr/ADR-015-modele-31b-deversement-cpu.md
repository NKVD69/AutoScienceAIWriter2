...
produit. Un document se rédige sur des semaines ; une section produite en un
quart d'heure au lieu d'une minute reste utilisable, une section médiocre ne
l'est pas.
...
5. **Le seuil `llm_max_ttft_ms` est un plafond de sécurité, pas un détecteur
   de rechargement.** Le temps au premier token est dominé par le traitement
   du prompt, qui croît avec le contexte : 3,7 s sur une invite courte, 13,5 s
   sur 700 tokens, et de l'ordre de 160 s attendues à 8 192 tokens. Un seuil
   serré se déclencherait en régime sain, un seuil large recouvrirait le coût
   d'un rechargement : les deux grandeurs ne se séparent pas. Le détecteur de
   rechargement est ailleurs et il est fiable — **l'état de résidence du
   modèle** (point 4). Le seuil est porté à 300 s et ne garde qu'un rôle de
   plafond contre un état pathologique.

   *Une rédaction antérieure de ce point fixait 15 s en affirmant que ce seuil
   séparait les deux régimes. La mesure du spike 02 l'a démenti.*

## Mesures qui fondent la décision

Relevé du 7 septembre 2026, `google/gemma-4-31b` Q8_0 résident sous LM Studio.

| Grandeur | Valeur | Conditions |
|---|---|---|
| VRAM occupée | 9 713 / 10 240 Mo | modèle résident |
| RAM occupée | 59,7 / 63,9 Go | modèle résident |
| Débit de génération | **2,3 tokens/s** | génération complète d'un `PlanTree` (1 453 s) |
| Débit apparent | 0,6 token/s | budget de 24 tokens — **non extrapolable**, le temps d'amorce y domine |
| Temps au premier token | 3,7 s / **13,5 s** | invite courte / invite de ~700 tokens |
| Débit d'amorce du prompt | ~52 tokens/s | ~160 s attendues à 8 192 tokens de contexte |
| Contexte maximal | 262 144 tokens | 8 192 configurés |

**Ordre de grandeur à connaître.** À 2,3 tokens/s, une section de 1 500 mots
(~2 000 tokens) demande environ **15 minutes**. Un mémoire de quarante sections
représente de l'ordre de **10 heures de génération cumulée**, hors relecture et
reprises. C'est le prix accepté ; il est consigné ici pour qu'il ne soit
redécouvert par personne.

> **Correction du 8 septembre 2026.** Une première rédaction de cet ADR
> annonçait 0,6 token/s, 55 minutes par section et 36 heures par mémoire. Ce
> chiffre provenait de `check_llm_latency.py`, qui génère 24 tokens : sur un
> budget aussi court, le temps d'amorce du prompt écrase le débit et la
> mesure n'est pas extrapolable. Le spike 02, sur une génération complète, en
> donne le quadruple. Les deux mesures sont exactes ; seule l'extrapolation
> de la première était fautive.
...
| Supprimer le seuil de TTFT | Le plafond garde une utilité contre un état pathologique, même s'il ne détecte pas un rechargement |
...
- **L'expérience d'interface change de nature.** Une section qui met un quart
  d'heure à se produire — et un plan complet vingt-cinq minutes, mesurées —
  impose un suivi de progression et une reprise après arrêt, non une attente
  synchrone. US-801 et US-DASH-001 doivent en tenir compte : le pont SSE n'est
  plus un confort.
...
de performance. Ce sont des **plafonds contre un état pathologique** ; le seul
détecteur fiable de rechargement est l'état de résidence. Tout nouveau seuil de
latence doit être introduit avec cette lecture — et avec une mesure de ce qu'il
prétend séparer, faute de quoi il séparera deux régimes qui se recouvrent.
